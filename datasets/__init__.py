...
def get_dataset(p):
    ...
    assert (config.data.dataset in [x.upper() for x in image_datasets]) \
        or (config.data.dataset in [x.upper() for x in Data.Generator.available_distributions]), \
        "Dataset not available: {}.\nCan choose from:\n(Image)\t{}\n(2d data)\t{}".format(config.data.dataset,
                                                                                          image_datasets,
                                                                                          Data.Generator.available_distributions)
    
    if config.data.dataset in [x.upper() for x in Data.Generator.available_distributions]:
        data_gen = Data.Generator(config.data.dataset, 
